...
class CommunityWeatherViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = CommunityWeather.objects.annotate(distance=Distance("location", user_location)).filter(distance__lte=4000).order_by('-date_reported','-time_reported','distance')[0:1]
        print(queryset)
        return queryset
